...
<script type="text/javascript">
				...
				$.ajax({
					...
					success:function(data){
						if(data.success){
							//刷新关联市场活动的列表
							showActivityList();
							//清除搜索框的信息，去掉√，清除activitySearchBody中的内容
							$("#aname").val("");
	...
	function showActivityList(){
		$.ajax({
			url:"workbench/clue/getActivityCLueById.do",
			data:{
				"clueId":"${c.id}"
			},
			dataType:"json",
			...
			success:function(data){
				...
				$.each(data,function(i,e){
					...
					html += '<td>'+e.endDate+'</td>';
					html += '<td>'+e.owner+'</td>';
					html += '<td><a href="javascript:void(0);" onclick="unbund(\''+e.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
				})
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/convert.jsp?id=${c.id}&fullname=${c.fullname }&company=${c.company }&appellation=${c.appellation }&owner=${c.owner }';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
