<%@page import="com.lixiang.crm.workbench.domain.Tran"%>
<%@page import="java.util.Set"%>
<%@page import="java.util.Map"%>
<%@page import="com.lixiang.crm.settings.domain.DicValue"%>
...
<script type="text/javascript">
	...
	*/
	...
	function changeIcon(stage,index1){
		...
		//如果当前阶段可能性为0，前7个一定是黑圈，后两个一个黑叉，一个红叉
		...
		//如果当前阶段可能性不为0，前7个是绿标，绿勾，黑圈，后两个都是黑叉
		}else{
			//遍历前7个
			for(var i=0;i<point;i++){
				if(i==index){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段的可能性为0，前7个为黑圈，后两个一个红叉，一个黑叉
			if("0".equals(currentPossibility)){
				for(int i=0;i<dvList.size();i++){
					...
					//如果遍历出来的阶段可能性为0，说明是后两个，后两个一个红叉，一个黑叉
			...
			//如果当前阶段的可能性不为0，前7个有可能为绿叉，绿勾，黑圈，后两个为黑叉
			}else{
				//准备当前阶段的下标
				int index = 0;
				for(int i=0;i<dvList.size();i++){
